...
from utils.memory import avoid_OOM, clear_load_data_cache
...
class local_dataset_processor:
  def __init__(self, loader, prepare_data_func):
    super().__init__()
    self.loader = loader
  ...
  def process(self, dataset, batch_size=500, num_proc=10):
    avoid_OOM()
    return dataset.map(self.prepare_data_func, batch_size=batch_size, remove_columns=dataset.column_names["train"], num_proc=num_proc)
  ...
  def process_save(self, raw_data_folder, save_folder, cache_file_name_map=None, batch_size=500, num_proc=10, num_writer = 1000, keep_in_memory = False):
    # # raw_data_folder must contain metadata.csv
    dataset = self.loader(data_dir=raw_data_folder, keep_in_memory=keep_in_memory, cache_dir = cache_path)
    if len(dataset) == 0:
      print(f"{raw_data_folder} ia an empty dataset")
      return
    ...
    if cache_file_name_map is not None:
      dataset = dataset.map(self.prepare_data_func, remove_columns=dataset.column_names, num_proc=num_proc,batch_size=batch_size, cache_file_name = str(cache_file_name_map / 'temp.arrow'), writer_batch_size=num_writer)
    else:
      dataset = dataset.map(self.prepare_data_func, remove_columns=dataset.column_names, num_proc=num_proc,batch_size=batch_size, writer_batch_size=num_writer)
    ...
    clear_load_data_cache(cache_path)


  def process_save_segment_mode(self, raw_data_folder, save_folder, cache_file_name_map=None, num_row=2500, batch_size=500, num_proc=10, num_writer=1000, keep_in_memory = False):
    # raw_data_folder must contain metadata.csv
    dataset = self.loader(data_dir=raw_data_folder, keep_in_memory=keep_in_memory, cache_dir = cache_path)
    ...
    for temp_dataset in yield_dataset:
      ...
      avoid_OOM()
      if cache_file_name_map is not None:
        temp_dataset = temp_dataset.map(self.prepare_data_func, remove_columns=temp_dataset.column_names, num_proc=num_proc,batch_size=batch_size, cache_file_name = str(cache_file_name_map / 'temp.arrow'), writer_batch_size=num_writer)
      else:
        temp_dataset = temp_dataset.map(self.prepare_data_func, remove_columns=temp_dataset.column_names, num_proc=num_proc,batch_size=batch_size, writer_batch_size=num_writer)
  ...
  def process_save_folder(self, data_folder, storage_folder, cache_file_name_map=None, batch_size=500, num_proc=10, num_writer=1000, keep_dataset_in_memory = False):
    ...
    # random mode for multi thread
    random.shuffle(need_to_process_folder)
...
if __name__ == '__main__':
  ...
  def prepare_dataset_with_label(batch):
    ...
    batch["input_features"] = processor(audio["array"], sampling_rate=16000, return_tensors='np').input_features[0]

    # encode target text to label ids
    # batch["labels"] = self.processor.tokenizer(batch["transcription"].lower()).input_ids
    batch["labels"] = processor.tokenizer(format_string(batch["transcription"])).input_ids
    
    # batch['change'] = batch['is_changed'] 
    return batch
  ...
  if with_label:
    loader = lambda data_dir, keep_in_memory, cache_dir : load_dataset("audiofolder", data_dir=data_dir, keep_in_memory=keep_in_memory, cache_dir = cache_dir)['train']

    prepare_data_func = prepare_dataset_with_label
    print("with label")
  else:
    loader = lambda data_dir, keep_in_memory, cache_dir : load_dataset("audiofolder", data_dir=data_dir, keep_in_memory=keep_in_memory, cache_dir = cache_dir, drop_metadata=True, drop_labels=True)['train']

    prepare_data_func = prepare_map_for_transcription
    print("without label")
  ...
  if '1' in task:
    ...
    data_process = local_dataset_processor(loader, prepare_data_func)
